...
from model.ait_vitdet import AITVitDetAttention, AITVitDetConfig
...
def compile(ait_model, shape_1, weights):
    ...
    Y, target, "./tmp", "VitDetAttention", constants=weights
) 
...
batch_size=15
input_size = 14
shape_1 = [batch_size, input_size, input_size, 384]


# codegen
target = detect_target()
# create AIT model
ait_model = AITVitDetAttention(mock_config, input_size)

# create pt model
pt_model = VitDetAttention(mock_config, input_size).cuda().half()
...
tensor_path = os.path.join(os.path.dirname(__file__), '..', 'saved_tensors', 'attention_input.pt')
